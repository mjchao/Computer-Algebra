...
public class LinkedList < E > implements Iterable < E > {
	...
	/**
	 * creates a <code>LinkedList</code> with the specified initial elements
	 * 
	 * @param initialElements			initial contents of this list
	 */
	public LinkedList( E[] initialElements ) {
		for ( int elementIdx = 0 ; elementIdx < initialElements.length ; elementIdx ++ ) {
			add( initialElements[ elementIdx ] );
		}
	}
	...
	/**
	 * advances the current element forward by one. this may advance to <code>null</code>
	 * if advance is called on the last element in the list
	 */
	public void advance() {
		
		//make sure there is currently an element
		if ( hasCurrent() ) {
	 ...
	 * <p>
	 * <u>you cannot compare a linkedlist to itself. that will screw the iterators up</u>
	 */
	@Override
	public boolean equals( Object anObject ) {
		...
		if ( anObject instanceof LinkedList < ? > ) {
			...
			for ( E elementInThisList : this ) {
				...
				if ( elementInThisList == null ^ elementToCompare == null ) {
					return false;
				}
				if ( elementInThisList != null ) {
